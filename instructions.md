...
- **Recover Internal Library**: selects the original retained `main:library` without accessing a missing file manager. Its result identifies the recovered path.

- **Cancel Library Copy**: available while a copy is queued, interrupted or failed. Run while stopped to clear the job and keep using the current storage.
...
Only `library/` is shared, including its ROM and firmware folders. Keep the hidden `.romm-storage` completion marker intact. Temporary copy files are removed after a successful copy. Saves, screenshots, artwork, database, passwords, configuration and synchronization keys stay in RomM's own storage and backup. The file manager does not need a public address for RomM to use it; an address is useful when you want to manage files through its web interface.
...
Stop RomM to interrupt a copy. It stops after the current file finishes. Start again to retry interrupted work in the same destination; the package replaces only its own partial staging files and keeps the source selected until the copy finishes. If the copy reports an error, automatic retries stop. While RomM is stopped, fix the reported problem and select the same destination in **Configure Library Storage** to retry explicitly. Other configuration actions wait until you finish or cancel the copy.

To cancel, stop RomM and run **Cancel Library Copy**. This action appears only while a copy is queued, interrupted or failed. This preserves the source and any partial destination files. After cancellation, use a new empty external folder or delete that partial folder in the file manager before starting a new copy. The cleanup action can remove cancelled internal copies.
